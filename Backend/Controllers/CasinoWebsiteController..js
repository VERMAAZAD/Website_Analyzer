...
exports.GetallUniqueDomains = async (req, res) => {
  try {
    ...
    const allTime = await CasinoWebsite.aggregate([
    ...
    const dailyStats = await CasinoWebsite.aggregate([
      {
        $match: {
          ...matchCondition,
...
exports.domainTraffic = async (req, res) => {
  try {
    ...
    const locationStats = await CasinoWebsite.aggregate([
      { $match: matchCondition },
      {
        $group: {
...
exports.GetTopCountries = async (req, res) => {
  try {
    ...
    const topCountries = await CasinoWebsite.aggregate([
      { $match: matchCondition },
      {
        $group: {
          _id: { country: "$location.country", visitor: "$visitorId" },
