...
const AdsWebsite = require("../Models/AdsWebsite");
...
exports.GetallUniqueDomains = async (req, res) => {
  try {
    ...
    const allTime = await AdsWebsite.aggregate([
    ...
    const dailyStats = await AdsWebsite.aggregate([
      {
        $match: {
          ...matchCondition,
...
exports.domainTraffic = async (req, res) => {
  try {
    ...
    const locationStats = await AdsWebsite.aggregate([
      { $match: matchCondition },
      {
        $group: {
...
exports.GetTopCountries = async (req, res) => {
  try {
    ...
    const topCountries = await AdsWebsite.aggregate([
      { $match: matchCondition },
      {
        $group: {
          _id: { country: "$location.country", visitor: "$visitorId" },
