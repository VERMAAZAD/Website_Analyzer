...
exports.saveScrapedData = async (req, res) => {
 ...
 try {
    ...
    const existing = await ScrapedGameSite.findOne({ domain: baseDomain, user: req.user._id });
    if (existing) {
      return res.status(400).json({ error: 'E11000: Duplicate domain for this user' });
    }

    const newSite = new ScrapedGameSite({
      domain: baseDomain,
      h1: data.h1,
      h2: data.h2,
      images: data.images,
...
exports.getAllScrapedSites = async (req, res) => {
 try {
    ...
    const sites = await ScrapedGameSite.find(query); 
    res.json(sites);
  } catch (err) {
...
exports.getScrapedSiteByDomain = async (req, res) => {
  ...
  try {
    ...
    const site = await ScrapedGameSite.findOne(query);
...
exports.getAllCategories = async (req, res) => {
  try {
    ...
    const categories = await ScrapedGameSite.distinct('brandCategory', {
      user: req.user._id,
...
exports.getCategoryCounts = async (req, res) => {
  try {
    ...
    const aggregation = await ScrapedGameSite.aggregate([
      {
        $match: {
          user: req.user._id,
          brandCategory: { $ne: null },
...
exports.getDomainsByCategory = async (req, res) => {
  try {
    ...
    const domains = await ScrapedGameSite.find({
...
exports.deleteScrapedSite = async (req, res) => {
  ...
  try {
    const result = await ScrapedGameSite.findOneAndDelete({
      domain: baseDomain,
      user: req.user._id
    });
...
exports.getErrorDomains = async (req, res) => {
  ...
  try {
       const errorDomains = await ScrapedGameSite.find(query)
...
exports.getExpiringDomains = async (req, res) => {
  try {
    ...
    if (expiredDomains.length > 0) {
      await ScrapedGameSite.deleteMany({ _id: { $in: expiredDomains } });
    }
...
const cleanDomain = domain.replace(/^https?:\/\//, '').replace(/^www\./, '').trim();
 ...
 const filter = req.user.role === 'admin'
    ...
    const updated = await ScrapedGameSite.findOneAndUpdate(
      filter,
      { note },
      { new: true }
    );
...
if (totalDomains > 0 && checkedTodayCount === totalDomains) {
  ...
  try {
   ...
   // Simulate human behavior
        ...
        const site = await ScrapedGameSite.findOne({ domain: fullUrl });
        if (site) {
...
exports.getUnindexedDomains = async (req, res) => {
  ...
  const sites = await ScrapedGameSite.find({ ...filter, isIndexedOnBing: false });
...
exports.saveHostingInfo = async (req, res) => {
  ...
  try {
    const site = await ScrapedGameSite.findOne({ domain });
...
exports.getHostingInfo = async (req, res) => {
  ...
  try {
    ...
    const site = await ScrapedGameSite.findOne(filter);
...
exports.updateDomainName = async (req, res) => {
  ...
  try {
    const site = await ScrapedGameSite.findOne({ domain: oldDomain });
    ...
    const existing = await ScrapedGameSite.findOne({ domain: newDomain });
    if (existing) {
