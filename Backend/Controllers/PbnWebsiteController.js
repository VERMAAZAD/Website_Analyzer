...
const PbnWebsite = require("../Models/PbnWebsite");
...
exports.GetallUniqueDomains = async (req, res) => {
  try {
    ...
    const allTime = await PbnWebsite.aggregate([
    ...
    const dailyStats = await PbnWebsite.aggregate([
      {
        $match: {
          ...matchCondition,
...
exports.domainTraffic = async (req, res) => {
  try {
    ...
    const locationStats = await PbnWebsite.aggregate([
      { $match: matchCondition },
      {
        $group: {
...
exports.GetTopCountries = async (req, res) => {
  try {
    ...
    const topCountries = await PbnWebsite.aggregate([
      { $match: matchCondition },
      {
        $group: {
          _id: { country: "$location.country", visitor: "$visitorId" },
